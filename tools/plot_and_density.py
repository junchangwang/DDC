...
bm_100m_c<N>_<algo>/ directories — disjoint bitmaps, AND result ≈ 0).
Emits a clean PDF showing AND op-only performance vs bitmap density
...
D_MIN = 0.02     # rightmost (c = 50) — within this window ComBit sits above
...
# One extra "transition worst-case" point on the density axis — CR's bitset→
# array AND extract peak.  count_a=16000 gives ~3900 set bits per 65 536-segment
# in the AND result (just under CR's 4096 array threshold), forcing CR to call
# bitset_extract_intersection_setbits_uint16 on every segment.  All 7 backends'
# numbers for this point come from bm_100m_t16000_<algo>/.
TRANSITION_COUNT_A = 16000     # count_a in the .bm dir name
TRANSITION_DENSITY = TRANSITION_COUNT_A / 65536.0   # ≈ 0.244
...
def load(csv_path: Path):
    """Returns two dicts:
       ...
       tran: {backend: median_ms} for the single TRANSITION_COUNT_A point
             (independent random bitmaps — CR's bitset→array extract worst case)
    Note: TRANSITION_COUNT_A (=16000) is NOT in the standard sweep, so any row
    with cardinality==16000 in the CSV is unambiguously from the transition
    dir (bm_100m_t16000_*) regardless of result_card."""
    std:  dict[tuple[str, int], float] = {}
    tran: dict[str, float] = {}
    standard_cs = {2, 5, 10, 20, 50}   # density ∈ {0.5, 0.2, 0.1, 0.05, 0.02}
    with csv_path.open() as f:
        for row in csv.DictReader(f):
            ...
            res_card = int(row["result_cardinality"])
            backend  = row["backend"]
            time_ms  = float(row["time_ms"])
            # TRANSITION_COUNT_A is never in standard_cs → unambiguous transition.
...
def write_dat(std_samples, tran_samples, dat_path: Path):
    ...
    row sits at TRANSITION_DENSITY (≈ 0.244, between c=4 and c=5 on the axis)."""
    rows = []
...
def write_gp(gp_path: Path, dat_path: Path, pdf_path: Path):
    ...
    # Pure density values on the x-axis — no cardinality labels.
    xtics = (
        f"('0.5' 0.5, "
        f"'{TRANSITION_DENSITY:.3f}' {TRANSITION_DENSITY}, "
        f"'0.2' 0.2, '0.1' 0.1, '0.05' 0.05, '0.02' 0.02)"
    )
...
# disjoint AND = empty result) don't crush the dense-end story.
set logscale y 10
set yrange [1:*]
...
# Annotation: highlight the density (≈ 0.244) where CR's bitset→array
# AND-result extract worst case sits.  Light vertical guide only — no text
...
def main():
    ...
    out_pdf  = Path(sys.argv[2] if len(sys.argv) > 2 else "and_density.pdf")
