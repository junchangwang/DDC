#!/usr/bin/env python3
"""
Benchmark ComBit (w8) across all cardinalities.

Generates ComBit compressed bitmap files if not present, then runs
benchmark_app with --backend combit and collects storage + operation timing.

Usage (on server):
    cd ~/lee/thesis/check/lee/combit
    python3 ../run_combit.py                          # default: all cards
    python3 ../run_combit.py --regen                  # force regenerate .bm files
    python3 ../run_combit.py -o my_results.csv        # custom output filename
    python3 ../run_combit.py --cards 2,100,1000       # specific cardinalities
    python3 ../run_combit.py --iterations 10          # more iterations
"""
...
ALGO_LABEL = "ComBit-w8"


def get_bm_dir(base_dir, card):
    return os.path.join(base_dir, "bitmap", f"bm_100m_c{card}_combit_w8")
...
def generate_bitmaps(base_dir, build_dir, force_regen, cards=None):
    ...
    print("=" * 55)
    print(" Step 1: Generating ComBit bitmaps")
    print("=" * 55)
    ...
    for card in cards:
        ...
        if os.path.isfile(done_file):
            print(f"  [SKIP] bm_100m_c{card}_combit (already exists)")
            continue

        print(f"  [GEN]  ComBit c={card} ...", end=" ", flush=True)
...
def parse_benchmark_output(output, card):
    """Extract storage + timing from benchmark_app stdout for ComBit."""
    results = []
    ...
    # ── ComBit storage breakdown (MB) ──
    cb_l3_mb = 0.0
    cb_l2_mb = 0.0
    cb_l1_mb = 0.0
    cb_total_mb = 0.0
    m = re.search(
        r'\[ComBit Storage\]\s*l3_bytes:\s*([\d.e+-]+)\s*MB'
    ...
    # ── Pure Ops (ComBit) ──
    pure_section = re.search(r'\[Pure Ops\] ComBIT', output)
    if pure_section:
...
def run_benchmarks(base_dir, build_dir, csv_path, cards=None, iterations=None):
    ...
    print("=" * 55)
    print(" Step 2: Running ComBit benchmarks")
    print(f" Cards      : {cards}")
    ...
    for card in cards:
        ...
        print(f"  [{count}/{total}] ComBit c={card} ...", end=" ", flush=True)
...
def main():
    parser = argparse.ArgumentParser(
        description="ComBit (w8) benchmark runner")
    parser.add_argument("--base-dir", default=".",
                        help="combit project root (default: .)")
    parser.add_argument("-o", "--output", default="results_combit.csv",
                        help="output CSV filename (default: results_combit.csv)")
    parser.add_argument("--regen", action="store_true",
                        help="force regenerate all ComBit .bm files")
