...
static void run_correctness_tests() {
    ...
    // --- Specification example -----------------------------------------
    {
        ...
        bool ok = true;
        // 6 words, one leading bit each: 110110
...
static void run_segmented_correctness_tests() {
    ...
    std::cout << "--- Segment Count Tests ---\n";
    {
        ...
        // 200000 / 65536 = 3.05..., so 4 segments
    ...
    // --- Cross-word-size AND on segmented ComBit -----------------------
    std::cout << "--- Segmented Cross-WS AND Tests ---\n";
    {
        auto a = generate_uniform(200000, 0.3, 1);
        auto b = generate_uniform(200000, 0.3, 2);

        // AND between different word sizes (same segmentation)
...
[[maybe_unused]] static void run_compression_analysis() {
    std::cout << "========================================\n"
    ...
    // --- Size breakdown for a representative case ----------------------
...
static void run_AND_performance_benchmarks() {
    ...
    // --- AND operation speed (ComBitBtv vs uncompressed) ---------------
    ...
    for (double d : densities) {
        ...
        double t8   = time_ms([&]{ auto r = a8  & b8;
                                    g_sink += r.compressed_size_bits(); }, ITER);
...
static void run_OR_performance_benchmarks() {
    ...
    // --- OR operation speed (ComBitBtv vs uncompressed) ----------------
...
static void run_XOR_performance_benchmarks() {
    ...
    // --- XOR operation speed (ComBitBtv vs uncompressed) ---------------
