#ifndef DDC_SIMD_UTIL_HPP
#define DDC_SIMD_UTIL_HPP

/// SIMD acceleration utilities for DDC.
///
...
enum SupportFlags : int {
    DDC_NONE   = 0,
    DDC_AVX2   = 1,
    DDC_AVX512 = 2,  // requires F + BW + VPOPCNTDQ
};
                     ...
                     uint16_t* dst, size_t n)
{
    ...
    if (hw & DDC_AVX512) { words_or_avx512(a, b, dst, n); return; }
    if (hw & DDC_AVX2)   { words_or_avx2(a, b, dst, n); return; }
#endif
...
#if DDC_IS_X64
    int hw = hardware_support();
    if (hw & DDC_AVX512) { words_and_avx512(a, b, dst, n); return; }
    if (hw & DDC_AVX2)   { words_and_avx2(a, b, dst, n); return; }
#endif
...
#if DDC_IS_X64
    int hw = hardware_support();
    if (hw & DDC_AVX512) { words_xor_avx512(a, b, dst, n); return; }
    if (hw & DDC_AVX2)   { words_xor_avx2(a, b, dst, n); return; }
#endif
...
#if DDC_IS_X64
    int hw = hardware_support();
    if (hw & DDC_AVX512) { words_andnot_avx512(a, b, dst, n); return; }
    if (hw & DDC_AVX2)   { words_andnot_avx2(a, b, dst, n); return; }
#endif
...
#if DDC_IS_X64
    int hw = hardware_support();
    if (hw & DDC_AVX512) { return words_popcount_avx512(data, n); }
    if (hw & DDC_AVX2)   { return words_popcount_avx2(data, n); }
#endif
