...
int ddc_dual_probe(char op, const DDCBtv& a, const DDCBtv& b) {
    ...
    // branchless double accumulator over fill lanes: acc_or collects set
    // bits (kills all-zero), acc_not collects cleared bits (kills all-ones)
...
int DDCBtv::uniform_class() const {
    ...
    if (state_ == State::Compressed) {
        ...
        return -1;   // literal-bearing compressed segments stay stored
    }
...
DDCBtv::popcount() const {
    ...
    if (bit_count_ % 8 != 0) {
        const size_t rem = bit_count_ % 8;
        // Decompressed state stores every word as a literal; is_last_word_literal()
        // walks L3/L2 and is only meaningful for Compressed.
        const bool last_lit =
            state_ == State::Decompressed ? true : is_last_word_literal();
        if (!last_lit) {
            if (l1_fill_ones_) count -= 8 - rem;   // ones-fill covers the pads
        } else if (l1_lit_count_ > 0) {
            // defensive: never count pad bits of a literal tail byte
...
DDC::compress(const std::vector<bool>& bits, bool l1_fill_ones,
    ...
    while (offset < bits.size()) {
        ...
        if (adaptive_l1_polarity && ddc_dual_enabled()) {
            // v6: per-WORD fill value (dual-fill); falls back to the classic
            // single-polarity form whenever only one fill kind is present
        ...
        if (adaptive_l1_polarity) {
            // per-segment fill value (paper 4.1): store the minority side
            size_t ones = 0;
...
DDC::decompress() const {
    if (sparse_form_) {
        // gap form: absent global segment == all-zero; ones_ids_ == all-ones
...
// v3.1: lazily build (and cache) the zero/ones segment masks — the per-op
// rebuild was 42-59%% of clustered OR time (stage profile, 8/2).
void
...
// materialise the canonical fixed-grid form from the gap form: every absent
// global segment becomes a make_all_fill zero segment (same object AND's
// legacy zero branch emits); every ones_ids_ entry becomes a make_all_fill
// ones segment.  Logically const — representation change only.
void
...
size_t
DDC::popcount() const {
    // gap form needs no materialisation: absent segments hold no bits and a
    // virtual all-ones segment holds exactly its bit width
    size_t count = 0;
    for (const auto& seg : segments_)
        count += seg.popcount();
    for (const auto& run : ones_runs_)
...
void DDCBtv::serialize_v4(std::ostream& os, bool is_last_seg) const {
    ...
    // deserialize_v4 derives the L4 byte count from bit_count; hollow
    // make_all_fill segments (ensure_flat's gap/ones materialisation) carry
    // an EMPTY l4_bits_, so pad with zero bytes up to the derived count.
    // Canonical segments have l4_bits_.size() == derived count — their
