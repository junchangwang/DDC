 ...
 * ComBit Encoder
 *
 * Encodes an uncompressed bitmap vector into ComBit (Compressed Bitvector) format.
 *
 ...
 * words.
 *
 * The result is an ComBitEncoding containing:
...
public:
     ...
     * @return       The ComBit-compressed encoding.
     */
    static ComBitEncoding encode(const std::vector<bool>& bits);
     ...
     * @return       The ComBit-compressed encoding.
     */
    static ComBitEncoding encode_words(const std::vector<Word>& words);

private:
    /// Flush a pending run of identical words into the encoding.
    /// Handles runs longer than MAX_FILL_LENGTH by emitting multiple
    /// fill words.
    static void flush_run(ComBitEncoding& enc, uint8_t fill_bit_val, size_t run_length);
};

} // namespace combit

#endif // COMBIT_ENCODER_HPP