// compress_tpch_q1.cpp — Generate ComBit, WAH, CRoaring, EWAH bitmaps for TPC-H Q1
//
// Reads raw .bm files from tpch_q1 dir (LSB-first packed bits) and
// generates compressed versions in 4 formats.
//
// Usage:
//   compress_tpch_q1 <input_dir> <output_base> <num_rows>
...
void compress_one(const std::string& raw_path,
    ...
    { auto h = bits_to_bitmap(&combit_be, bits);   combit_be.Serialize(*h, combit_path); }
...
int main(int argc, char* argv[]) {
    ...
    for (int v = 0; v <= 2; v++) {
        ...
        compress_one(input_dir + "/" + rel,
                     ...
                     croaring_dir + "/" + rel,
                     ewah_dir + "/" + rel,
                     combit_be, wah_be, croaring_be, ewah_be, rf_stats);
    }
    ...
    for (int v = 0; v <= 1; v++) {
        ...
        compress_one(input_dir + "/" + rel,
                     ...
                     croaring_dir + "/" + rel,
                     ewah_dir + "/" + rel,
                     combit_be, wah_be, croaring_be, ewah_be, ls_stats);
    }
    ...
    for (int id : shipdate_ids) {
        ...
        compress_one(input_dir + "/" + rel,
                     ...
                     croaring_dir + "/" + rel,
                     ewah_dir + "/" + rel,
                     combit_be, wah_be, croaring_be, ewah_be, ship_stats, false);
        progress++;
    ...
    total_stats.combit_total = rf_stats.combit_total + ls_stats.combit_total + ship_stats.combit_total;
    ...
    for (auto* dir : {&combit_dir, &wah_dir, &croaring_dir, &ewah_dir}) {
