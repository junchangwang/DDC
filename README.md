...
- `src/core/ddc/`: the DDC compression library (see `src/core/ddc/README.md`). `DDCBtv` is one compressed segment (L1 literal bytes / L2 word-control bits / L3 byte-control bits / L4 top-control bits). `DDC` is the whole bitvector. `DDCN` is the depth-parameterised variant (L2-L5) used by the hierarchy-depth study.
